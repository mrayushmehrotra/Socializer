  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await axios.post("/api/gemini", {
        prompt:
          input +
          "you are the best youtuber in the world yeah, you google gemini you have to help this guy for creating the best hashtags so that he can reach more audience in less time and his channel should keep growing, just write the best Hashtags without any extra BS, else it will be rejected",
      });
  ...
  return (
    <UIWrapper classname="flex flex-col h-screen ">
      <div className="fixed top-0 left-0 w-full flex items-center justify-between z-20  backdrop-blur-md border-b border-gray-700 px-6 py-4">
        {/* Title on left */}
        <h1 className="text-2xl md:text-4xl font-bold">✨ Hashtags</h1>
        <SecondNav />
      </div>
